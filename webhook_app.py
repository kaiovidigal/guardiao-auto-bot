...
v7.7.0 flow-keeper+fibo30 (bagging+jitter) + force-rotate + entry-guard

- Fecha G0 pelo ÚLTIMO dígito 1..4 no ÚLTIMO par de (), [] ou {} (também （）【】)
- Ensemble: ProfoundSim + (curto, médio, longo, n-gram) + Fibo10 + FiboFino (5,8,13,21,34) + FIBO-30 bagging + T30
- Anti-viés (dominância/repetições) + empurrão de entropia
- Slate de opções (top-K) + ClassCooldown + Force-Rotate Guard
- NeuroController FLOW (auto-tuning min_conf + peso neural) e kill-switch curto
...
# Force-rotate (anti-lock final)
DIVERSITY_FORCE_ROTATE_REPS = int(os.getenv("DIVERSITY_FORCE_ROTATE_REPS", "3"))
DIVERSITY_GAP_FORCED       = float(os.getenv("DIVERSITY_GAP_FORCED", "0.05"))
...
F30_BAG_SIZE  = int(os.getenv("F30_BAG_SIZE",  "12"))     # subset por decisão (8..16)
F30_JITTER    = float(os.getenv("F30_JITTER",   "0.003"))  # 0.0..0.01
...
# ================== APP ==================
app = FastAPI(title="GuardiAo Auto Bot (webhook)", version="7.7.0-flow-keeper")
...
def _score_text()->str:
    ...
    tot=g+l; acc=(g/max(1,tot)*100.0) if tot>0 else 0.0
...
# ===== ProfoundSim (neural offline, sintético) =====
...
# ===== Force-Rotate Guard (anti-lock final) =====
def _enforce_diversity_guard(ranked: List[Tuple[int,float]]) -> Tuple[List[Tuple[int,float]], str]:
    """
    Se o mesmo número foi sugerido repetidamente e a diferença pro 2º é pequena,
    rotaciona para o 2º (ou 3º) para evitar viciar — mantendo assertividade.
    """
    note = "none"
    if not ranked: 
        return ranked, note
    ...
    if last.isdigit() and int(last) == ranked[0][0] and reps >= DIVERSITY_FORCE_ROTATE_REPS and gap <= DIVERSITY_GAP_FORCED:
        alt_idx = 1
        if len(ranked) >= 3 and (ranked[1][1] - ranked[2][1]) <= 0.01:
            alt_idx = 1 if ranked[1][1] >= ranked[2][1] else 2
        ranked[0], ranked[alt_idx] = ranked[alt_idx], ranked[0]
        note = f"force-rotate from {last} (reps={reps}, gap={gap:.3f})"
    return ranked, note
...
# ===== decisão (ensemble + antibias + slate + cooldown + force-rotate) =====
def _e_fibo30_vote(tail: List[int], wins: Tuple[int, ...]=FIBO30_WINDOWS) -> Dict[int, float]:
    if not tail:
    ...
    for k in wins:
        ...
        if not win:
            continue
        best = max((1,2,3,4), key=lambda c: win.count(c))
        w = 1.0 / max(1.0, math.sqrt(float(k)))
        votes[best] += w
    return _norm(votes)

def _e_fibo30_bagging(tail: List[int], bag_size: int = F30_BAG_SIZE) -> Dict[int, float]:
    if not tail:
        return {1:0.25, 2:0.25, 3:0.25, 4:0.25}
    seed = len(tail)*131 + int(time.time())//7
    rng = random.Random(seed)
    bag = tuple(sorted(rng.sample(FIBO30_WINDOWS, k=min(bag_size, len(FIBO30_WINDOWS)))))
    probs = _e_fibo30_vote(tail, bag)
    if F30_JITTER > 0:
        for c in (1,2,3,4):
            probs[c] = max(0.0, probs[c] + (rng.random()-0.5)*2.0*F30_JITTER)
        probs = _norm(probs)
    return probs
...
def _neural_decide_slate()->Tuple[List[Tuple[int,float]], int, Dict[int,float], float, str, dict]:
    ...
    p_fb =_e_fibo10(tail)
    p_ff =_e_fibo_multi(tail)
    p_f30=_e_fibo30_bagging(tail)     # << FIBO-30 bagging+jitter
    p_t30=_e_t30()
    p_nn =_neural_probs(tail)

    # mistura: parte neural + estatística + fibos + relógio
    ...
    for c in (1,2,3,4):
        ...
        fibs = 0.40*p_fb.get(c,0)+0.25*p_ff.get(c,0)+0.35*p_f30.get(c,0)
        mix[c] = wneu*p_nn.get(c,0) + w_stat*stat + w_f*fibs + w_t*p_t30.get(c,0)

    mix=_conf_floor(_norm(mix), 0.30, 0.95)

    # check Neural×Fibo-30: reforça acordo, corta leve divergência
    nn_top = max(p_nn, key=p_nn.get)
    f30_top = max(p_f30, key=p_f30.get)
    agree  = int(nn_top == f30_top)
    mix2 = {}
    for c in (1,2,3,4):
    ...
    # force-rotate guard
    ranked, rotate_note = _enforce_diversity_guard(ranked)

    gap01 = (ranked[0][1] - ranked[1][1]) if len(ranked)>=2 else ranked[0][1]
    temp, wneu_cur, _=_load_neural_params()
    reason=f"ProfoundSim+Fibo30(w={wneu_cur:.2f},T={temp:.2f}) · check={'OK' if agree else 'mix'}"
    if rotate_note != "none":
        reason += f" · {rotate_note}"
...
async def debug_cfg():
    ...
    return {
        ...
        "force_rotate": {"reps": DIVERSITY_FORCE_ROTATE_REPS, "gap_forced": DIVERSITY_GAP_FORCED}
    }
...
async def webhook(token: str, request: Request):
    ...
    if RX_ENTRADA.search(text):
        ...
        # 🚫 Se já existe pendência, NÃO fecha e NÃO abre nova: aguarda fechamento
        pend=_pending_get()
        if pend:
            if SHOW_DEBUG:
                await tg_send(TARGET_CHANNEL,
                              "DEBUG: Nova ENTRADA recebida com pendência aberta — aguardando FECHAMENTO (não fecharei como X).")
            return {"ok": True, "waiting_previous_close": True}
