...
app = FastAPI(title="guardiao-auto-bot (GEN Hibrido)", version="2.7.1")
...
# ======== TIMEOUT ÚNICO (pedido): inicia quando houver 2 observados ========
...
def migrate_db():
    ...
    cur.execute("""CREATE TABLE IF NOT EXISTS pending (
        ...
        last_conf_long  REAL,
        d_final INTEGER           -- << só esse deadline é usado agora
    )""")
...
async def webhook(token: str, request: Request):
    ...
    # Sempre tenta destravar pendência antiga por timeout único
    ...
    # Abre pendência nova (sem d_final; ele só nasce quando virem DOIS observados)
