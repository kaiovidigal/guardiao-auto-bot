...
v7.0-patch (G1-only, fecha G0 pelo último número entre parênteses quando não vier "Sequência")
...
# App
# ------------------------------------------------------
app = FastAPI(title="GuardiAo Auto Bot (webhook)", version="7.0-patch")
...
# ------------------------------------------------------
# IA proxy simples (n-gram + frequências)
# ------------------------------------------------------
...
# NOVO: captura parênteses no fim da msg
RX_PAREN_END = re.compile(r"\(([^\)]*)\)\s*$")

def _parse_seq_list(text:str)->List[int]:
    m=RX_SEQ.search(text or "")
    return [int(x) for x in RX_NUMS.findall(m.group(1))] if m else []
...
def _parse_paren_pair(text:str, need:int=2)->List[int]:
    m=RX_PAREN_END.search(text or "")
    if not m: return []
    nums=[int(x) for x in re.findall(r"[1-4]", m.group(1))]
    return nums[:need]

def _parse_paren_last_one(text:str)->Optional[int]:
    """Último número entre parênteses, para fechar G0 quando não vier 'Sequência'."""
    m=RX_PAREN_END.search(text or "")
...
async def webhook(token: str, request: Request):
    ...
    # 2) FECHAMENTO (fecha por "Sequência" ou, na falta, pelo último número entre parênteses)
    if RX_FECHA.search(text):
        ...
        if pend:
            ...
            # (a) tenta pegar do "Sequência: a | b"
            obs_pair = _parse_seq_pair(text, need=min(2, MAX_GALE+1))
            if obs_pair:
                _pending_seen_append(obs_pair, need=min(2, MAX_GALE+1))

            # (b) usa o último número dos parênteses para FECHAR G0 quando não vier "Sequência"
            last_paren = _parse_paren_last_one(text)
            if not obs_pair and last_paren is not None:
                _pending_seen_append([last_paren], need=min(2, MAX_GALE+1))

            # (c) sempre alimenta timeline com o que vier entre parênteses
            extra_tail = _parse_paren_pair(text, need=min(2, MAX_GALE+1))
            if extra_tail:
            ...
            # Se G0 != sugerido e temos espaço para G1, aguarda próxima
            ...
            # Fecha se GREEN no G0 ou já temos G1 observado
    ...
    if RX_ENTRADA.search(text):
        ...
        # fecha pendência anterior se esquecida
        pend=_pending_get()
        if pend:
            seen=[s for s in (pend["seen"] or "").split("-") if s]
