...
- SEND_ANALYZING      (default True)       # controla envio do "⏳ Analisando..."
...
SEND_ANALYZING   = os.getenv("SEND_ANALYZING", "True").strip().lower() == "true"
...
# --------- FECHAMENTO ESTRITO por parênteses () no FINAL da mensagem ---------
...
def _parse_close_digit_strict(text: str) -> Optional[int]:
    """
    Pega APENAS o número 1..4 que esteja dentro do ÚLTIMO par de parênteses '()'
    no FINAL da mensagem do canal-fonte. Ex.: '✅ GREEN (3)' -> 3 ; 'RED ❌ (4)' -> 4
    """
    t = _strip_noise(text)
...
async def webhook(token: str, request: Request):
    ...
    # 2) APOSTA ENCERRADA / GREEN / RED — FECHAMENTO IMEDIATO POR () ESTRITO
    if RX_FECHA.search(text) or RX_GREEN.search(text) or RX_RED.search(text):
        ...
        if pend:
            ...
            # captura do canal-fonte: número 1..4 no ÚLTIMO () no fim do texto
    ...
    # 3) ENTRADA CONFIRMADA (com dedupe + “Analisando...” auto-delete opcional)
