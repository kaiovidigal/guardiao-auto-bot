...
v7.3.2-controller+antibias (G0-only; ProfoundSim + NeuroController + Anti-bias + Plano B)

- Fecha G0 pelo último número entre parênteses (ex.: GREEN (3)/RED (4))
- IA Profunda Híbrida (ProfoundSim) + especialistas estatísticos
- NeuroController: calibra risco (epsilon, min_conf, cool-down, regime)
- Anti-viés/diversidade: limita dominância do mesmo número + empurra entropia
- Plano B: publica candidato secundário quando gap é pequeno
- Aprendizado leve (EMA) a cada GREEN/LOSS
...
ENTROPY_PUSH    = float(os.getenv("ANTIENTROPY_PUSH", "0.35"))

ENABLE_SECONDARY     = os.getenv("ENABLE_SECONDARY", "true").strip().lower() == "true"
SECONDARY_GAP_MAX    = float(os.getenv("SECONDARY_GAP_MAX", "0.09"))
SECONDARY_CONF_MIN   = float(os.getenv("SECONDARY_CONF_MIN", "0.23"))
...
# ================== APP ==================
app = FastAPI(title="GuardiAo Auto Bot (webhook)", version="7.3.2-controller+antibias")
...
# ========= meta KV =========
def _kv_get(key:str, default:str=""):
    con=_con(); row=con.execute("SELECT v FROM meta WHERE k=?", (key,)).fetchone(); con.close()
    return (row["v"] if row and row["v"] is not None else default)

def _kv_set(key:str, val:str):
    con=_con(); con.execute("INSERT OR REPLACE INTO meta(k,v) VALUES(?,?)", (key, str(val))); con.commit(); con.close()
...
def _pending_open(suggested:int):
    ...
    con.commit(); con.close()
    # anti-viés: rastrear repetição do mesmo sugerido
    last = _kv_get("last_suggested", "")
    reps = int(_kv_get("same_suggested_reps", "0"))
    if last and last.isdigit() and int(last)==int(suggested):
        reps += 1
    else:
        reps = 1
...
def _antibias_adjust(mix:Dict[int,float])->Tuple[Dict[int,float],dict]:
    ...
    if share > DIV_MAX_SHARE:
        ...
        reason_parts.append(f"share{dom}={share:.2f}>lim")
    last = _kv_get("last_suggested","")
    if last and last.isdigit():
        last_int=int(last)
        if same_reps >= DIV_MAX_SAME:
            adjusted[last_int] = max(0.0, adjusted.get(last_int,0.0) * 0.70)
            reason_parts.append(f"same_reps={same_reps}")
    if ENTROPY_PUSH>0:
        adjusted = _entropy_mix(adjusted, ENTROPY_PUSH)
        reason_parts.append(f"H{ENTROPY_PUSH:.2f}")

    adjusted=_conf_floor(_norm(adjusted), 0.30, 0.95)
    diag.update({
        "dominant": dom, "dominant_share": round(share,4),
        "same_suggested_reps": same_reps,
        "applied": " & ".join(reason_parts) if reason_parts else "none"
    })
    return adjusted, diag

def _secondary_pick(mix:Dict[int,float], top:int, gap:float)->int:
    if not ENABLE_SECONDARY: return -1
    if gap > SECONDARY_GAP_MAX: return -1
    ranked = sorted(mix.items(), key=lambda kv: kv[1], reverse=True)
    if len(ranked)<2: return -1
    sec, psec = ranked[1]
    if psec < SECONDARY_CONF_MIN: return -1
    if sec==top: return -1
    return int(sec)
...
# ============== Decisão final (neural + especialistas + anti-viés) ==============
def _neural_decide()->Tuple[int,float,int,Dict[int,float],float,str,dict,int,float]:
    ...
    base = {}
    for c in (1,2,3,4):
        base[c] = wneu*pn.get(c,0) + 0.40*rest*p1.get(c,0) + 0.30*rest*p2.get(c,0) + 0.20*rest*p3.get(c,0) + 0.10*rest*p4.get(c,0)
    base = _conf_floor(_norm(base), 0.30, 0.95)

    adj, ab_diag = _antibias_adjust(base)

    r = sorted(adj.items(), key=lambda kv: kv[1], reverse=True)
    best = int(r[0][0]); conf=float(r[0][1])
    gap = float(r[0][1]-r[1][1]) if len(r)>=2 else float(r[0][1])
    secondary = _secondary_pick(adj, best, gap)
    second_conf = float(r[1][1]) if len(r)>=2 else 0.0

    temp, wneu_cur, _ = _load_neural_params()
    reason = f"ProfoundSim(w={wneu_cur:.2f},T={temp:.2f})"
    return best, conf, _timeline_size(), adj, gap, reason, ab_diag, secondary, second_conf
...
async def debug_cfg():
    ...
    dom  = max(freq, key=freq.get); share=freq[dom]
    antibias = {
        "config": {"lastk":DIV_LASTK,"max_share":DIV_MAX_SHARE,"max_same":DIV_MAX_SAME,"entropy_push":ENTROPY_PUSH},
        ...
        "secondary_conf_min": SECONDARY_CONF_MIN
    }
    return {
...
async def webhook(token: str, request: Request):
    ...
    if RX_ENTRADA.search(text):
        ...
        # sugestão (neural+especialistas+antiviés)
        best, conf, samples, mix, gap, reason, ab_diag, secondary, sec_conf = _neural_decide()
        ...
        if opened:
            ...
            sec_line = ""
            if ENABLE_SECONDARY and secondary>0 and secondary!=suggested:
                sec_line = f"\n🅱️ <b>Plano B:</b> {secondary} • conf≈{sec_conf*100:.1f}% (Δ≈{gap*100:.1f}pp)"
            ...
            txt=(f"🤖 <b>IA SUGERE</b> — <b>{suggested}</b>\n"
                 ...
                 f"♻️ <i>anti-viés:</i> dom={ab_diag['dominant']}({ab_diag['dominant_share']*100:.1f}%), rep={ab_diag['same_suggested_reps']} • {ab_diag['applied']}{sec_line}\n"
