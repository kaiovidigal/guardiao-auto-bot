...
v7.4.3-flow  (G0-only; ProfoundSim + Fibo + T30 + NeuroController FLOW)
- Fecha G0 pelo último número entre parênteses
- IA Profunda Híbrida (ProfoundSim) + especialistas (Fibo, janelas, T+30min)
- NeuroController **FLOW**: sem cooldown, corte por confiança (min_conf)
- Reforço leve (EMA) por classe
- Anti-trava: timeout de pendência (fecha LOSS G0 X)
- Dedupe por conteúdo (entrada/analisando/fechamento)
...
# ================== APP ==================
app = FastAPI(title="GuardiAo Auto Bot (webhook)", version="7.4.3-flow")
...
# “Fibo10” – reforça o número mais frequente nas últimas 10/21/34/55
...
# “T30” – leve de-bias a cada ~30min (usa clock)
def _e_t30()->Dict[int,float]:
    m=int(datetime.datetime.utcnow().timestamp()//1800)  # janela 30min
...
# força modo fluxo contínuo
def _ctl_force_flow():
    _ctl_save(eps=CTL_EPSILON, minc=CTL_MIN_CONF, coolN=999999, coolS=1, thr=0, reg="flow")

def _ctl_decide(best:int, conf:float, mix:Dict[int,float], gap:float)->Tuple[bool,str,int]:
    _ctl_force_flow()  # garante sem cooldown
    eps, minc, _, _, _, reg = _ctl_load()

    if conf < minc:
        return False, f"conf {conf:.2f} < min {minc:.2f} (modo={reg})", -1
    ...
    return True, f"exploit (gap={gap:.3f}, modo={reg})", int(best)

# ===== decisão (neural + especialistas) =====
def _neural_decide()->Tuple[int,float,int,Dict[int,float],float,str]:
    ...
    rest=1.0 - wneu
    w_stat=0.65*rest; w_f=0.20*rest; w_t=0.15*rest
    mix={}
    for c in (1,2,3,4):
        stat = 0.40*p1.get(c,0)+0.30*p2.get(c,0)+0.20*p3.get(c,0)+0.10*p4.get(c,0)
        mix[c] = wneu*pn.get(c,0) + w_stat*stat + w_f*pf.get(c,0) + w_t*pt.get(c,0)
...
async def webhook(token: str, request: Request):
    ...
    if RX_ENTRADA.search(text):
        ...
        if not play:
            ...
            # **não** segura o fluxo – apenas informa e segue
        ...
        if opened:
            ...
            if analyzing_id is not None: await tg_delete(TARGET_CHANNEL, analyzing_id)
            if SHOW_DEBUG: await tg_send(TARGET_CHANNEL, f"DEBUG: Entrada aberta (FLOW).")
