...
# Endpoint do webhook usa o PRÓPRIO token do bot na URL: /webhook/<TG_BOT_TOKEN>
...
# ========= FASTAPI =========
app = FastAPI(title="Guardião Híbrido", version="1.1.0")
...
@app.get("/")
async def root():
    return {"ok": True, "detail": "Webhook ativo Guardião Híbrido (token na URL)"}

# ROTA usa o token do bot
...
async def webhook(bot_token: str, request: Request):
    ...
    # GREEN / RED registram no timeline
    U = t.upper()
    if "GREEN" in U or "RED" in U:
        nums = re.findall(r"[1-4]", t)
        if nums:
            append_timeline(int(nums[0]))
        return {"ok": True, "event": "GREEN" if "GREEN" in U else "RED"}

    # Quando vem “ENTRADA CONFIRMADA”, IA sugere e replica
    if "ENTRADA CONFIRMADA" in U:
