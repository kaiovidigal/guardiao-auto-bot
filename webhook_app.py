...
# =========================
# Qualidade PRO (moderado+)
# =========================
WINDOW = 1200          # ↑ amostra (Amostra≈N vai subir conforme timeline cresce)
DECAY  = 0.986         # memória levemente mais longa

# n-gram (puxa mais o contexto longo, mantendo estabilidade)
W4, W3, W2, W1 = 0.46, 0.28, 0.16, 0.10

# gaps (menos falso top1 colado)
GAP_SOFT = 0.018       # desempate técnico (usa fallback por frequência)
GAP_MIN  = 0.080       # gap mínimo para aceitar top1 na predição

# priors (mistura n-gram + histórico de padrão/estratégia)
ALPHA, BETA, GAMMA = 1.08, 0.72, 0.45

# amostra mínima só para habilitar “fire IA” (NÃO bloqueia o sinal do canal)
MIN_SAMPLES = 800
...
# ============================================================
# Helper: formata sequência de conferência (2 vistos + hit)
# ============================================================
...
def _ia_acc_days_wrapper():
    try:
        return _ia_acc_days(7)
    except:
        return (0,0,0.0)
...
def _adaptive_phase() -> str:
    ...
    s = _current_samples()
    _g, _l, acc7 = _ia_acc_days_wrapper()
...
# =========================
# Heurística extra: top-2 da cauda (k=50) — leve boost
# =========================
def tail_top2_boost(tail: List[int], k:int=50) -> Dict[int, float]:
    boosts = {1:1.00, 2:1.00, 3:1.00, 4:1.00}
    if not tail:
        return boosts
    tail_k = tail[-k:] if len(tail) >= k else tail[:]
    c = Counter(tail_k).most_common(2)
    if len(c) >= 1:
        boosts[c[0][0]] = 1.05  # top1
    if len(c) >= 2:
        boosts[c[1][0]] = 1.02  # top2
    return boosts
...
def suggest_number(base: List[int], pattern_key: str, strategy: Optional[str], after_num: Optional[int]) -> Tuple[Optional[int],float,int,Dict[int,float]]:
    ...
    boost_tail = tail_top2_boost(tail, k=50)
    ...
    # Anti-repetição fraca (não bloqueia, só troca para top2 quando top1≈top2)
    last = query_one("SELECT suggested, announced FROM pending_outcome ORDER BY id DESC LIMIT 1")
    if last and number is not None and int(last["suggested"]) == int(number):
...
async def webhook(token: str, request: Request):
    ...
    # 3) ENTRADA CONFIRMADA — processa G0 da nova janela (sempre entra no canal)
    ...
    if number is None:
        # Mesmo se IA abstiver por qualidade, o fluxo do canal segue com pendência? Aqui mantemos:
        # Se quiser SEMPRE abrir, mesmo sem number, remova este retorno e defina um fallback (ex.: top1 puro de post).
        await ia2_process_once()
