...
def is_entrada_branco(raw: str) -> bool:
    ...
    if not has_branco: return False
...
def _load_counters():
    try:
        with open(COUNTERS_PATH, "r") as f: return json.load(f)
    except Exception: return {"date": datetime.now().strftime("%Y-%m-%d"), "green": 0, "loss": 0}
def _save_counters(d): open(COUNTERS_PATH,"w").write(json.dumps(d))
def _reset_if_new_day():
    c = _load_counters(); today = datetime.now().strftime("%Y-%m-%d")
    if c.get("date") != today: c = {"date": today, "green": 0, "loss": 0}; _save_counters(c)
    return c
def contabilizar(r):
    c = _reset_if_new_day(); c["green" if r=="GREEN" else "loss"] += 1; _save_counters(c); return c
def get_status_msg():
    c = _reset_if_new_day(); return f"📊 *Parcial do dia* ({c['date']}):\n✅ GREEN: {c['green']}\n❌ LOSS: {c['loss']}"
...
def classificar_resultado(txt: str) -> Optional[str]:
    ...
    if vitoria and branco and ("protecao" not in t and "como protecao" not in t): return "GREEN_VALIDO"
    if any(p in t for p in ["derrota","loss","perdeu","perda","nao bateu","nao deu","falhou"]): return "LOSS"
    if any(p in t for p in ["vitoria de primeira","vitoria com","gale","g1","g 1","g2","g 2"]): return "LOSS"
    if "green" in t and not branco: return "LOSS"
    return None

# IA analítica
def _get_openai_client():
    if not ANALYTICS_MODE or not OPENAI_API_KEY: return None
    try: return OpenAI(api_key=OPENAI_API_KEY)
    except Exception as e: logging.error(f"OpenAI client error: {e}"); return None
...
def _carregar_eventos():
    ...
    try:
        with open(HISTORICO_PATH,"r") as f:
            for line in f:
                try: eventos.append(json.loads(line))
                except: pass
    except FileNotFoundError: pass
    return eventos

def _horario(h): return h[11:13] if h and len(h)>=13 else "??"

def _sinal_para_placar_deltas(eventos):
    deltas=[]; last=None
    for e in eventos:
        if e.get("tipo")=="entrada": last=e.get("hora")
            ...
            try:
                ...
                if t2>=t1: deltas.append(t2-t1); last=None
            except: pass
    return deltas
...
async def gerar_analise_ia_e_postar():
    ...
    if not client:
        await send_telegram_message(CANAL_DESTINO_ID,"⚠️ IA desativada ou sem OPENAI_API_KEY."); return
    ev=_carregar_eventos()
    if not ev: await send_telegram_message(CANAL_DESTINO_ID,"⚠️ Sem dados ainda para analisar."); return
    por_hora_green={}; g=l=0
    for e in ev:
        if e.get("tipo")=="resultado":
            if e.get("resultado")=="GREEN": g+=1; por_hora_green[_horario(e.get("hora"))]=por_hora_green.get(_horario(e.get("hora")),0)+1
            elif e.get("resultado")=="LOSS": l+=1
    deltas=_sinal_para_placar_deltas(ev); med=int(sorted(deltas)[len(deltas)//2]) if deltas else 0
    resumo=json.dumps({"greens_por_hora":por_hora_green,"greens_total":g,"losses_total":l,"mediana_segundos_sinal_para_placar":med},ensure_ascii=False,indent=2)
    try:
        r=client.chat.completions.create(model="gpt-4o-mini",messages=[{"role":"system","content":"Seja conciso e direto."},{"role":"user","content":f"Analise estes dados e traga top horários, janela de pareamento e taxa aproximada:\n{resumo}"}],temperature=0.2)
...
def _append_bounded(lst,val,maxlen=200):
    lst.append(val)
    if len(lst)>maxlen: del lst[:len(lst)-maxlen]
def _predict_next_white_eta():
    last=learn_state.get("last_white_ts")
    if not last: return None,None,None
    try: hh_last=datetime.fromtimestamp(float(last)).strftime("%H")
    except: hh_last=None
    med_h=None
    if hh_last:
        med_h=_med_or_none(learn_state.get("white_gaps_by_hour",{}).get(hh_last,[]))
    med_g=_med_or_none(learn_state.get("white_gaps",[]))
    if med_h: return float(last)+med_h, med_h, "hora"
    if med_g: return float(last)+med_g, med_g, "global"
    return None,None,None
def _since_last_white():
    lw=learn_state.get("last_white_ts"); return (time.time()-float(lw)) if lw else None
def _hour_stats(hh):
    slot=learn_state.get("by_hour",{}).get(hh,{"g":0,"l":0}); g,l=slot.get("g",0),slot.get("l",0); tot=g+l
    return (g/tot if tot>0 else 0.0), tot
def _recent_perf(n=12):
    ev=_carregar_eventos(); res=[e for e in ev if e.get("tipo")=="resultado"]; res=res[-n:] if len(res)>n else res
...
def _confidence_for_entry(text, now_ts):
    ...
    ev=_carregar_eventos(); streak=[e.get("resultado") for e in ev if e.get("tipo")=="resultado"][-3:]
    if len(streak)==3 and all(s=="LOSS" for s in streak): conf*=0.9
...
async def webhook(webhook_token: str, request: Request):
    ...
    if tnorm.startswith("/aprendizado"):
        deltas=learn_state.get("deltas",[]); med=int(median(deltas)) if deltas else 0
        byh=learn_state.get("by_hour",{}); top=sorted(((h,v.get("g",0),v.get("l",0)) for h,v in byh.items()), key=lambda x:(x[1]-x[2]), reverse=True)[:3]
        linhas=[f"{h}h → G:{g} / L:{l}" for h,g,l in top] or ["(sem dados)"]
        await send_telegram_message(CANAL_DESTINO_ID, f"🧠 *Aprendizado*\n• Mediana Δ sinal→placar: {med}s\n• RESULT_WINDOW_SECONDS: {RESULT_WINDOW_SECONDS}s\n• Top horários (G-L):\n- " + "\n- ".join(linhas))
        return {"ok":True}
    if tnorm.startswith("/confstatus"):
        ...
        await send_telegram_message(CANAL_DESTINO_ID, "🔎 *Confiança agora*\n"
    ...
    if tnorm.startswith("/branco_tempo"):
        ...
        await send_telegram_message(CANAL_DESTINO_ID, "⏱️ *Tempo entre BRANCOS*\n"
        ...
        if last_signal_msg_id and (now-last_signal_time<=RESULT_WINDOW_SECONDS):
            ...
            if resultado=="GREEN_VALIDO":
                ...
                if last_w:
                    ...
                    if gap>0:
                        lst=learn_state.setdefault("white_gaps",[]); _append_bounded(lst,gap,200); learn_state["white_gaps"]=lst
                ...
                learn_state["last_white_ts"]=now_ts; _save_learn()
    ...
    if is_entrada_branco(text):
        ...
        now=time.time()
        if COOLDOWN_SECONDS and now-last_signal_time<COOLDOWN_SECONDS: return {"ok":True,"action":"ignored_cooldown"}
